...
<!-- POINTER ONLY. This file answers two questions and nothing else:
     ...
     this template at session end — exactly like development-state.md and sprint-state.md. The full
     rewrite is a hard reset: anything outside these fixed slots is dropped, so drift cannot accumulate.
     The human may set feature, tier, and gate_policy. Hooks read sprint_path/mode/tier. -->

feature: {{feature-slug or none}}
name: {{Human-readable name}}
mode: {{plan|deliver}}
tier: {{Hotfix|Nano|Lite|Standard|Meticulous|Full}}
phase: {{planning|executing|review|closed}}
gate_policy: {{auto-proceed|review-on-open-issues|review-all}}
sprint_path: {{workstream/sprints/<sprint-id> or none}}
started: {{YYYY-MM-DD}}
