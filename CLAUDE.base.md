# Forge — Operating Manual (managed)

<!-- TEMPLATE-MANAGED. Imported into every agent via CLAUDE.md (`@CLAUDE.base.md`). Do NOT hand-edit
     this inside a project: `forge-sync` overwrites it from the upstream template. Project-specific
     facts live in the Project Stack block of CLAUDE.md, never here. -->

This manual is the single source of truth for the workspace layout, the control plane (modes,
tiers, gates), and the rules every agent obeys. Keep it lean. Deep process lives in skills; artifact
shape lives in templates; project specifics live in the Project Stack block in `CLAUDE.md`.

Forge is technology-agnostic. It builds webapps, services, scripts, integrations, libraries —
anything that is code. The only project-specific part of this scaffold is the Project Stack block in
`CLAUDE.md` and the artifact templates. Everything else carries over unchanged and is kept current by
`forge-sync` (the `.claude/skills/forge-sync` skill).
...
- Subagents run in isolated context. They see nothing except `CLAUDE.md` (which imports this
  manual), their own role prompt, and whatever the orchestrator hands them in the delegation
  prompt. **All shared memory lives on disk.** If the orchestrator does not name a path, the
...
```
CLAUDE.md                         ← per-project entry point: Project Stack (seeded) + @CLAUDE.base.md import
CLAUDE.base.md                    ← this managed operating manual (forge-sync keeps it current)
.gitattributes                    ← forces LF so hooks/scripts stay runnable across OSes
.forge/                           ← structure version control:
  version.json                    ←   structural version + file-hash manifest (managed)
  CHANGELOG.md                    ←   scaffold changes + migration notes (managed)
  instance.json                   ←   THIS project's upstream link (instance-owned; never synced)
scripts/forge-manifest.sh         ← (re)generate & verify the manifest; doubles as a scaffold doctor
scripts/forge-sync.sh             ← compare against the upstream template & pull structural updates
.claude/agents/                   ← orchestrator + planning/ + delivery/ role definitions
.claude/skills/                   ← plan-workflow, deliver-workflow, project-init, forge-sync, reference/
.claude/settings.json             ← managed harness config (hooks). Per-project overrides → settings.local.json
